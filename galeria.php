...
<div class="col-lg-9 col-md-7">
        ...
        $query = $pdo->query("SELECT * FROM produtos where (nome LIKE '$buscar' or palavras like '$buscar') and ativo = 'Sim' and estoque > 0 order by data desc");
        }else if($valor_inicial != ""){
            $query = $pdo->query("SELECT * FROM produtos where valor >= '$valor_inicial' and valor <= '$valor_final' and ativo = 'Sim' and estoque > 0 order by data desc");
        }

        else{
            $query = $pdo->query("SELECT * FROM produtos where sub_categoria = '$id_subcategoria' and ativo = 'Sim' and estoque > 0 order by data desc");
        }
        $res = $query->fetchAll(PDO::FETCH_ASSOC);
   ...
   //BUSCAR O TOTAL DE REGISTROS PARA PAGINAR
          ...
          if($promocao == 'Sim'){
            ...
            <a style="background-image: url('img/produtos/<?php echo $imagem ?>');" class="publicacao-single" href="produto-<?php echo $nome_url ?>">
